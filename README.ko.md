...
2. **언어 선택**: 우측 상단 헤더의 토글 버튼(🌐 KO/EN/JA/ZH/ES/DE)을 눌러 원하는 언어로 전체 대시보드 화면을 전환합니다.
3. **에버노트 로그인**: `🔑 로그인 인증 받기`를 누르면 팝업되는 콘솔 창과 브라우저를 통해 로그인을 완료합니다 (완료 시 `token_bk.db` 생성).
4. **전체 백업 실행**: `🚀 원클릭 전체백업`을 실행하여 동기화, ENEX 추출, 마크다운 변환을 순차적으로 진행합니다.
5. **산출물 확인**: `📁 로컬 백업 폴더` 버튼을 누르면 변환된 마크다운 폴더가 열립니다.
...
* **다국어(6개국어) 시스템**: 한국어, 영어, 일본어, 중국어, 스페인어, 독일어를 완전히 지원하며, 네이티브 폴더 선택창의 상단 캡션 정보도 사용자 언어로 실시간 동적 현지화됩니다.
* **마크다운 및 첨부파일 변환**:
...
* **통합 뷰어 및 중복 정리기**: 마크다운 뷰어인 `MD Browser`와 중복 파일 정리기(`Duplicate Note Cleaner`)가 대시보드 서버 내에 완전히 내장되어 단일 포트(5001번 포트의 `/browser/`)에서 원클릭으로 기동 및 연동됩니다.
...
├── i18n/                 # 다국어(ko, en, ja, zh, es, de) JSON 사전 폴더
